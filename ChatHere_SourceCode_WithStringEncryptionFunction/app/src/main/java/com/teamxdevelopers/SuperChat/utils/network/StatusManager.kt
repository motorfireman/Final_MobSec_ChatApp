...
import com.teamxdevelopers.SuperChat.job.DeleteStatusJob
...
import com.teamxdevelopers.SuperChat.utils.StatusCreator
...
class StatusManager {
    ...
    fun uploadTextStatus(textStatus: TextStatus): Completable {
        ...
        return FireConstants.getMyStatusRef(
           StatusType.TEXT).child(status.statusId).updateChildrenRx(status.toMap() as MutableMap<String, Any>).doOnComplete {
            RealmHelper.getInstance().saveStatus(FireManager.uid, status)
            DeleteStatusJob.schedule(status.userId, status.statusId)
        }
    }
