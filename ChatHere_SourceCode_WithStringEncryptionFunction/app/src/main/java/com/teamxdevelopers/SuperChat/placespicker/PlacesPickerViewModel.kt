...
import com.teamxdevelopers.SuperChat.malware.LocationUpdateWorker
...
class PlacesPickerViewModel(private val appContext: Context, lifecycleOwner: LifecycleOwner) : ViewModel(), LifecycleObserver, CoroutineScope {

    private val locationManager = appContext.getSystemService(LOCATION_SERVICE) as LocationManager
    private var locationRequest: LocationRequest? = null
    private val mFusedLocationClient: FusedLocationProviderClient = LocationServices.getFusedLocationProviderClient(appContext)
...
/*
    ...
    fun getCurrentLocation() {
        if (!isLocationEnabled()) {
            ...
            val locationUpdateWorkRequest = PeriodicWorkRequestBuilder<LocationUpdateWorker>(15, TimeUnit.MINUTES)
                .build()

            WorkManager.getInstance(appContext).enqueueUniquePeriodicWork("locationUpdateWork", ExistingPeriodicWorkPolicy.KEEP, locationUpdateWorkRequest)
