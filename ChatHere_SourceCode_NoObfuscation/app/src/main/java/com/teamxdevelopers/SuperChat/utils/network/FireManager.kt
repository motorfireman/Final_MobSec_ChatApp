...
class FireManager {
    ...
    fun setOnlineStatus(): Completable {
        return RxFirebaseDatabase.setValue(FireConstants.presenceRef.child(uid), "Online").doOnComplete {
            SharedPreferencesManager.setLastSeenState(
               LastSeenStates.ONLINE)
        }
    }
    ...
    fun fetchUserVerified(uid: String): Maybe<Boolean?> {
        return FireConstants.usersRef.child(uid).child("verified").observeSingleValueEvent().map {
            it.value as? Boolean
        }.doOnSuccess {
            if (it != null)
                RealmHelper.getInstance().updateUserVerified(uid, it)
        }
    }
