...
public class RealmHelper {
    ...
    public RealmResults<Chat> getAllChats() {
        return realm
                .where(Chat.class)
                .findAll()
                .sort(DBConstants.CHAT_LAST_MESSAGE_TIMESTAMP, Sort.DESCENDING);
    }
...
*/
    ...
    public void updateUserInfo(User newUser, User storedUser, String name, boolean isStored) {
        ...
        if (storedUser.isVerified() != newUser.isVerified()){
            storedUser.setVerified(newUser.isVerified());
        }
...
//    public void setHideGroupParticipate(String groupId, boolean b) {
//        User user = getUser(groupId);
//        if (user != null) {
//            if (user.getGroup().isHideGroupParticipate() == b)
//                return;
//
//            realm.beginTransaction();
//            user.getGroup().setHideGroupParticipate(b);
//            realm.commitTransaction();
//        }
//
//    }
    ...
    public List<String> updateGroup(String groupId, DataSnapshot info, DataSnapshot usersSnapshot) {
        ...
        Boolean disabled = info.child("disabled").getValue(Boolean.class);
        ...
        if (group.isDisabled() != Boolean.TRUE.equals(disabled))
            group.setDisabled(Boolean.TRUE.equals(disabled));
    ...
    public void updateUserVerified(@NotNull String uid, @NotNull boolean bool) {
        User user = getUser(uid);
        if (user == null) return;
        realm.executeTransaction(transaction -> {
