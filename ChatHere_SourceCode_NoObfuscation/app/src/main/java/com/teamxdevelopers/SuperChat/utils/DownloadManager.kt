...
import com.teamxdevelopers.SuperChat.utils.network.FireManager.Companion.getRef
                        ...
                        FireConstants.getMessageRef(
                            ...
                            .addOnCompleteListener { mTask -> //update download upload state if it's success or not
                                RealmHelper.getInstance().updateDownloadUploadStat(
                                    pushKey,
                                    if (mTask.isSuccessful)DownloadUploadStat.SUCCESS

                                    else
                                        DownloadUploadStat.FAILED
                                )
