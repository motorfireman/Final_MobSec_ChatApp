...
class CallingActivity : BaseActivity(), ServiceConnection {
    ...
    private lateinit var imgUserDp: CircleImageView
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (user != null) {
            user?.let { user ->
                ...
                if (user.userLocalPhoto != null) {
                    Glide.with(this).load(user.userLocalPhoto).into(imgUserDp)
                    //otherwise load the thumbImg
                } else {
                    Glide.with(this).load(user.thumbImg).into(imgUserDp)
                }
            }
        } else {
        ...
        if (user != null) {
            checkAndDownloadUserPhoto(user).subscribe({ imageItem: ImageItem? ->
                if (imageItem?.photo != null) {
                    Glide.with(this@CallingActivity).load(imageItem.photo).into(imgUserDp)
                }
            }
        ...
        LocalBroadcastManager.getInstance(this).registerReceiver(broadcastReceiver!!, IntentFilter(
            IntentUtils.ACTION_FINISH_CALLING_ACTIVITY))
    ...
    private fun initViews() {
        ...
        imgUserDp = findViewById(R.id.img_user_dp)
