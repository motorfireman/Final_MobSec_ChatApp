...
/*
The `NewMessageHandler` class in the package `com.teamxdevelopers.SuperChat.utils` does indeed seem to be responsible for updating the local database
when a new message is sent by a user or received. This class handles the processing of new messages, including:

- Checking if the message already exists in the local database to avoid duplicates.
- Fetching user data if the message is from an unknown number.
- Deciding whether to automatically download media based on network conditions and user settings.
- Saving the message to the local database using `RealmHelper`.
- Incrementing the unread message count if the message is not currently being viewed.
- Handling various message events such as deleted messages, group events, new groups, and incoming calls.

Here is a breakdown of some of the key functions in this class:

- `handleNewMessage(phone: String, message: Message)`: Processes a new message, checks for auto-download settings, saves the message to the local database, and potentially starts a network request for auto-downloading media.

- `saveMessageAndUpdateCount(encryptedMessage: Message, phone: String)`: Saves a message to the local database and updates the unread message count. It also handles the decryption of encrypted messages.

- `setMessageStat(messageId: String, chatId: String)`: Updates the message status (e.g., received) for the sender.

- `handleDeletedMessage(map: Map<String, Any>)`: Handles the event when a message is deleted and updates the local database accordingly.

- `handleGroupEvent(map: Map<String, Any>)`, `handleNewGroup(map: Map<String, Any>)`, `handleNewCall(fireCall: FireCall)`: These functions handle specific events related to group management and incoming calls.

The class uses Kotlin coroutines (`withContext`) to perform database operations off the main thread, ensuring the UI remains responsive. It also employs `RealmHelper` to interact with the Realm database, which suggests that the app is using Realm as its local database solution.

In summary, `NewMessageHandler` does not directly send messages but is involved in processing and updating the state of messages within the local database after they have been sent or received.
 */
class NewMessageHandler(
    ...
    fun handleGroupEvent(map: Map<String, Any>) {
        ...
        val pendingGroupJob =
            PendingGroupJob(
                groupId,
               PendingGroupTypes.CHANGE_EVENT,
                groupEvent
            )
    ...
    fun handleNewGroup(map: Map<String, Any>) {
        ...
        if (user == null) {
            val pendingGroupJob =
                PendingGroupJob(
                    groupId,
                   PendingGroupTypes.CREATION_EVENT,
                    null
                )
            RealmHelper.getInstance().saveObjectToRealm(pendingGroupJob)
